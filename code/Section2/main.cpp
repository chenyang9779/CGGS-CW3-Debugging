...
void callback_function()
{
    ...
    for (int i = 0; i < numSpheres; i++)
    {
        for (int j = i + 1; j < numSpheres; j++)
        {
            ...
            if (sqrDistance > 4.1 * sqrRadius)
            { // Otherwise the objects are too close and it will overshoot (tunnelling)
                forceDirection.normalize();
                forces.row(i) = forces.row(i) + forceDirection * gravityConstant / sqrDistance;
                forces.row(j) = forces.row(j) - forceDirection * gravityConstant / sqrDistance;
            }
        }
    }
